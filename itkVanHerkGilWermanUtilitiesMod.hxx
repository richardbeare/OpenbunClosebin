 ...
 *
 *=========================================================================*/
#ifndef __itkVanHerkGilWermanUtilitiesMod_hxx
#define __itkVanHerkGilWermanUtilitiesMod_hxx
...
template <class PixelType, class TFunction>
void CollapseMorph(unsigned len, unsigned KernLen,
                   std::vector<PixelType> &pixbuffer,
                   ...
                   std::vector<PixelType> &rExtBuffer)
{
  ...
  if ( size <= KernLen / 2 )
    ...
    for ( unsigned j = KernLen / 2, k = KernLen / 2 + KernLen / 2, l = KernLen / 2 - KernLen / 2;
      ...
      PixelType V1 = fExtBuffer[k];
      PixelType V2 = rExtBuffer[l];
      pixbuffer[j] = m_TF(V1, V2);
      }
    // line end -- involves reseting the end of the reverse
    // extreme array
...
template< class TImage, class TBres, class TFunction1, class TFunction2, class TLine >
void DoFace2Op(typename TImage::ConstPointer input,
               typename TImage::Pointer output,
               typename TImage::PixelType border,
               TLine line,
               const typename TBres::OffsetArray LineOffsets,
               ...
               const typename TImage::RegionType face)
{
  ...
  for ( unsigned int it = 0; it < face.GetNumberOfPixels(); it++ )
    ...
    if ( FillLineBuffer< TImage, TBres, TLine >(input, Ind, NormLine, tol, LineOffsets,
      ...
      pixbuffer[0] = border;
      pixbuffer[len + 1] = border;
      FillForwardExt< typename TImage::PixelType, TFunction1 >(pixbuffer, fExtBuffer, KernLen, len + 2);
      FillReverseExt< typename TImage::PixelType, TFunction1 >(pixbuffer, rExtBuffer, KernLen, len + 2);
      // now compute result of first stage
      CollapseMorph<typename TImage::PixelType, TFunction1>(len, KernLen, pixbuffer,
                                                            fExtBuffer, rExtBuffer);

      // stage 2
      // confirm whether we need to reset the borders - I don't think o
      //pixbuffer[0] = border2;
      //pixbuffer[len + 1] = border2;
      FillForwardExt< typename TImage::PixelType, TFunction2 >(pixbuffer, fExtBuffer, KernLen, len + 2);
      FillReverseExt< typename TImage::PixelType, TFunction2 >(pixbuffer, rExtBuffer, KernLen, len + 2);
      // now compute result of first stage
      CollapseMorph<typename TImage::PixelType, TFunction2>(len, KernLen, pixbuffer,
                                                            fExtBuffer, rExtBuffer);


      CopyLineToImageOp< TImage, TBres, TFunction2 >(output, Ind, LineOffsets, pixbuffer, start, end);
      }
    }
}
