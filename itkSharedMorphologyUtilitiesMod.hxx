 ...
 *
 *=========================================================================*/
#ifndef __itkSharedMorphologyUtilitiesMod_hxx
#define __itkSharedMorphologyUtilitiesMod_hxx

#include "itkSharedMorphologyUtilitiesMod.h"
...
template< class TLine >
unsigned int GetLinePixels2(const TLine line, unsigned int target)
{
...
template< class TImage, class TBres, class TFunction >
void CopyLineToImageOp(const typename TImage::Pointer output,
                       const typename TImage::IndexType StartIndex,
                       const typename TBres::OffsetArray LineOffsets,
                       std::vector<typename TImage::PixelType> & outbuffer,
                       const unsigned start,
                       const unsigned end)
{
  // this one replaces output with TFunction(in, out);
  unsigned size = end - start + 1;
  TFunction m_TF;
  for ( unsigned i = 0; i < size; i++ )
    {
    // itkAssertInDebugAndIgnoreInReleaseMacro(start + i >= 0);
    itkAssertInDebugAndIgnoreInReleaseMacro( start + i < LineOffsets.size() );
    typename TImage::IndexType ind = StartIndex + LineOffsets[start + i];
    output->SetPixel(ind, m_TF(output->GetPixel(ind), outbuffer[i + 1]));  //compat
    }
}
